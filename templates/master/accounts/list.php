...
<script>
    ...
    async function loadAccounts() {
        ...
        try {
            const res = await fetch(MASTER_API_URL, { headers });
            ...
            if (!data.accounts || data.accounts.length === 0) {
                tbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted" style="text-align:center; padding:20px;">No accounts found</td></tr>';
                return;
            }
            ...
            document.querySelector('tbody').innerHTML = '<tr><td colspan="7" class="text-danger text-center" style="text-align:center;">Failed to load accounts: ' + err.message + '</td></tr>';
        }
    }
    ...
    function manageAccount(id) {
        ...
        showNotification('Edit Account feature coming soon', 'info');
    }
    ...
    async function terminateAccount(username) {
        // Logic remains similar but using showConfirm if available
        if (window.showConfirm) {
            showConfirm(
                'Terminate Account?',
        ...
        } else if (confirm('Terminate ' + username + '? All data will be lost.')) {
            performTerminate(username);
        }
    }
