...
<script>
    fetch('/public/api/master/packages', { headers: { 'Authorization': 'Bearer ' + (sessionStorage.getItem('token') || document.querySelector('meta[name="api-token"]')?.content) } })
        .then(r => r.json())
        .then(data => {
            ...
            if (data.packages && data.packages.length > 0) {
                ...
                <tr>
                    ...
                    <td>
                        <small title="Sub/Parked/Addon">
                            ${p.max_subdomains || 0}/${p.max_parked_domains || 0}/${p.max_addon_domains || 0}
                        </small>
                    </td>
                    <td>
                        <a href="edit?id=${p.id}" class="btn btn-secondary btn-sm">Edit</a>
        ...
        .catch(e => {
            console.error(e);
            document.querySelector('#packagesTable tbody').innerHTML = '<tr><td colspan="6" class="text-danger" style="text-align:center;">Failed to load data</td></tr>';
        });
