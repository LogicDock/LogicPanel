...
if (!$serviceId) {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                const res = await fetch('/public/api/services', {
                    ...
                    <div class="service-card">
                        <div class="service-card-header">
                            <div class="service-card-icon">
                                ${app.type === 'nodejs' ? '<i data-lucide="hexagon"></i>' : '<i data-lucide="codepen"></i>'}
                            </div>
...
<div class="app-manager-container" style="padding:0; height:calc(100vh - 80px);">
    <div id="terminal-wrapper"
        style="width:100%; height:100%; display:flex; flex-direction:column; background:#1e1e1e;">
        <div id="terminal-header"
            style="background:#252526; color:#ccc; padding:8px 15px; font-size:12px; display:flex; justify-content:space-between; align-items:center;">
            <div>
                <i data-lucide="terminal" style="width:14px; display:inline-block; vertical-align:middle;"></i>
                <span id="term-status">Initializing...</span>
            </div>
            <div>
                <button onclick="window.location.reload()"
                    style="background:transparent; border:none; color:#ccc; cursor:pointer;" title="Reload Terminal"><i
                        data-lucide="refresh-cw" style="width:14px;"></i></button>
            </div>
        </div>
...
<script>
    ...
    async function authenticateAndConnect() {
        ...
        try {
            ...
            // Adjust URL for browser (localhost vs remote)
            let wsUrl = gateway_url;
            if (window.location.hostname !== 'localhost') {
                const urlObj = new URL(gateway_url); // ws://localhost:3002
