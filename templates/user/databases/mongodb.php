...
<script>
                ...
                list.innerHTML = filtered.map(db => `
                    <tr>
                        <td>
                            ...
                            <div style="font-size:11px;color:var(--text-secondary);">${db.host}:${db.port}</div>
                        </td>
                        <td><span style="color:var(--text-secondary);">-</span></td>
                        <td><span class="user-badge">${db.user}</span></td>
                        <td class="actions-col">
                            <div style="display:flex;gap:5px;justify-content:flex-end;">
                                <button class="btn btn-sm btn-secondary" title="Copy Connection String" onclick="copyConnString('${db.host}', '${db.user}', '${db.password}', '${db.name}')">
    ...
    function copyConnString(host, user, encryptedPass, dbname) {
        try {
            const pass = encryptedPass ? atob(encryptedPass) : '';
            // Use browser hostname if internal docker host is used
            let connHost = host;
            if (connHost === 'lp-mongo-mother' || connHost === 'lp-mysql-mother' || connHost === 'lp-postgres-mother') {
                connHost = window.location.hostname;
            }

            const safeUser = encodeURIComponent(user);
            const safePass = encodeURIComponent(pass);

            // Auth source is admin because provisioner creates user in admin db
            const str = `mongodb://${safeUser}:${safePass}@${connHost}:27017/${dbname}?authSource=admin`;
            ...
            showNotification('Failed to process credentials', 'error');
        }
    }
        ...
        */
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Use browser hostname if internal docker host is used
                let connHost = data.database.host;
                if (connHost === 'lp-mongo-mother') { connHost = window.location.hostname; }

                // Encode credentials to handle special characters (Base64 +, /, =)
                const safeUser = encodeURIComponent(data.database.user);
                const safePass = encodeURIComponent(data.database.password);

                // Construct Conn String for Modal (Auth source is admin because provisioner creates user in admin db)
                const connStr = `mongodb://${safeUser}:${safePass}@${connHost}:27017/${data.database.name}?authSource=admin`;
